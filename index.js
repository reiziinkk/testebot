...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `ESPERO QUE ESSE @${num.split('@')[0]} FILHA DA PUTA NUNCA MAIS VOLTE`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭ESPERA UM POUCO CARALHO❬❗❭',
				success: '️❬ ✔ ❭ JA FOI FILHA DA PUTA',
				error: {
			...
			const ownerNumber = ["5521976394573@s.whatsapp.net"] 
...
case 'lofi':
			...
			case 'mapa': 
				...
				case 'audio':
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('texto muito grande seu filha da puta😤')
